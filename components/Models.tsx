...
export const Models: FC<modelsProps> = ({ models }) => {
    ...
    const [selectedSegment, setSelectedSegment] = useState<ApiModelsResponse['segment'] | undefined>(undefined)
  ...
  return (
    <div>
      <div className='flex items-center justify-evenly gap-4'>
        <button onClick={resetFilters} disabled={selectedSegment === undefined}>Todos</button>
        {Array.from(SEGMENTS).map(segment => (
          <button key={segment} onClick={() => filterBySegment(segment)} disabled={segment === selectedSegment}>{segment}</button>
        ))}
      </div>
     {filtered.map(model =>
